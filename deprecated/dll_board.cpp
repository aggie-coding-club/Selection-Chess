...
bool DLLBoard::apply(std::shared_ptr<PieceMove> _move) {
    ...
    Tile* startTile = getTile(DMtoSA(_move->m_startPos), false);
    Tile* endTile = getTile(DMtoSA(_move->m_endPos), false);
...
bool DLLBoard::apply(std::shared_ptr<TileMove> _move) {
    ...
    Coords moveSelFirstIC = DMtoSA(_move->m_selFirst) + m_minCoords;
    Coords moveSelSecondIC = DMtoSA(_move->m_selSecond) + m_minCoords;
