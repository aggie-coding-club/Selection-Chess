...
void SparseBoard::updateExtrema(const DModCoords& _new) {
...
void SparseBoard::init(const std::string _sfen) {
    ...
    if (m_grid_size > SIZE_MAX / m_grid_size) {
        std::cerr << "Board is too large to be stored as SparseBoard!" << std::endl;
    ...
    tdout << "SparseBoard::init counted " << m_grid_size << " size required and will now init" << std::endl;

    // some arbitrary starting point
    const DModCoords STARTING_CORNER = std::make_pair(0, 0);
    m_minCoords = m_maxCoords = STARTING_CORNER; // TODO: remove STARTING CORNER, and just use m_minCoords?

    DModCoords currentFR = STARTING_CORNER;
    for (i = 0; i < _sfen.length() && _sfen[i] != ' '; i++) {
        const char c = _sfen[i];
        if (c == '/') { // Next row
...
bool SparseBoard::updatePieceInPL(PieceEnum _piece, DModCoords _oldLocation, DModCoords _newLocation) {
...
bool SparseBoard::removePieceFromPL(PieceEnum _piece, DModCoords _location) {
    for (int i = 0; i < m_pieceLocations[_piece].size(); i++) { // loop for all pieces of type _piece
...
bool SparseBoard::apply(std::shared_ptr<Move> _move) {
...
bool SparseBoard::undo(std::shared_ptr<Move> _move) {
...
bool SparseBoard::apply(std::shared_ptr<PieceMove> _move) {
    // tdout << "applying PieceMove " << _move->algebraic() << std::endl;
    // tdout << getAsciiBoard() << std::endl;;
    DModCoords startCoords = SAtoAB(DMtoSA(_move->m_startPos));
    DModCoords endCoords = SAtoAB(DMtoSA(_move->m_endPos));
...
bool SparseBoard::undo(std::shared_ptr<PieceMove> _move) {
...
bool SparseBoard::apply(std::shared_ptr<TileMove> _move) {
    tdout << "applying tileMove " << _move->algebraic() << std::endl;
    DModCoords moveSelFirst = SAtoAB(DMtoSA(_move->m_selFirst));
    DModCoords moveSelSecond = SAtoAB(DMtoSA(_move->m_selSecond));

    // ----------- Cut out the selection ----------- //
    // Save old coords in case we have to undo the cut operation
    DModCoords oldMin = m_minCoords, oldMax = m_maxCoords; 
    ...
    DModCoords moveDestFirst = SAtoAB(DMtoSA(_move->m_destFirst));
    DModCoords moveDestSecond = SAtoAB(DMtoSA(dModDestSecond));
    ...
    for (DModCoords i = moveDestFirst; i.second != moveDestSecond.second + 1; ++i.second) { // iterate rows
...
bool SparseBoard::undo(std::shared_ptr<TileMove> _move) {
...
bool SparseBoard::apply(std::shared_ptr<TileDeletion> _move) {
    tdout << "applying TileDeletion move " << _move->algebraic() << std::endl;
    for (DModCoords& dModDeletion : _move->m_deleteCoords) {
        DModCoords deletionCoords = SAtoAB(DMtoSA(dModDeletion));
    ...
    DModCoords updatedMin = std::make_pair( //TODO: this is repeated code from getSelection -- consider making this into a function, as well as its counterpart in apply, too
    ...
    DModCoords updatedMax = std::make_pair(
    ...
    DModCoords minUpdate = updatedMin - m_minCoords; // since we are cutting tiles, updatedMin >= m_minCoords aka change in min is positive.
...
bool SparseBoard::undo(std::shared_ptr<TileDeletion> _move) {
    ...
    for (DModCoords& dModAddition : _move->m_deleteCoords) {
        ...
        DModCoords addedCoords = SAtoAB(DMtoSA(dModAddition));
...
bool SparseBoard::isLegal(std::shared_ptr<Move> _move) {
    switch (_move->m_type) {
    case PIECE_MOVE:
...
bool SparseBoard::isLegal(std::shared_ptr<PieceMove> _move) {
    //TODO: implement
    return true;
}
bool SparseBoard::isLegal(std::shared_ptr<TileMove> _move) {
...
bool SparseBoard::isLegal(std::shared_ptr<TileDeletion> _move) {
...
DModCoords SparseBoard::SAtoAB(Coords _extern) const {
    DModCoords intern(_extern);
...
Coords SparseBoard::ABtoSA(DModCoords _intern) const {
...
std::string SparseBoard::dumpAsciiArray() const {
    std::string result = "[";
...
int SparseBoard::staticEvaluation() {
...
bool SparseBoard::moveIsFromMO(std::shared_ptr<Move> _move, const MoveOption& _mo) {
    switch (_mo.m_type) {
    case LEAP_MO_TYPE:
...
std::vector<std::unique_ptr<Move>> SparseBoard::getMovesFromMO(DModCoords& _pieceCoords, const MoveOption& _mo) {
    switch (_mo.m_type) {
    case LEAP_MO_TYPE:
...
bool SparseBoard::moveIsFromMO(std::shared_ptr<Move> _move, const LeapMoveOption& _mo) {
    // TODO: implement
    return false;
}

std::vector<std::unique_ptr<Move>> SparseBoard::getMovesFromMO(DModCoords& _pieceCoords, const LeapMoveOption& _mo) {
...
bool SparseBoard::moveIsFromMO(std::shared_ptr<Move> _move, const SlideMoveOption& _mo) {
    // TODO: implement
...
std::vector<std::unique_ptr<Move>> SparseBoard::getMovesFromMO(DModCoords& _pieceCoords, const SlideMoveOption& _mo) { // FIXME: I think I broke this lol. Going to fix next commit probably
...
std::vector<std::unique_ptr<Move>> SparseBoard::getMoves(PieceColor _color) {
    ...
    for (int pieceType = (_color==WHITE ? W_PAWN : B_PAWN); pieceType < NUM_PIECE_TYPES*2+1; pieceType+=2) { // iterate over pieces of _color in piece list
        ...
        for (DModCoords pieceCoords : m_pieceLocations[pieceType]) { // for all coords of pieces of this type
    ...
    // TODO: gross iteration over the entire space, would be nice if I only iterated over tiles.
    DModCoords startCoords = m_minCoords;
    // iterate over rows
...
StandardArray SparseBoard::getSelection(const DModCoords& _bl, const DModCoords& _tr, bool _cut) { // TODO: consider merging with standardArray() ?
    ...
    size_t i = 0; // current index of sa
    DModCoords coords = std::make_pair(_bl.first, _tr.second);
    // iterate over rows
    ...
    if (_cut) {
        // tdout << "minCoords=" << m_minCoords << " maxCoords=" << m_maxCoords << "before cut," << std::endl;
        // it is possible we cut off the min/max, so we need to update it accordingly
        DModCoords updatedMin = std::make_pair(
            nextTileByColOrder(std::make_pair(m_minCoords.first, 0), false).first, 
            nextTileByRowOrder(std::make_pair(0, m_minCoords.second), false).second
        );
        DModCoords updatedMax = std::make_pair(
...
void SparseBoard::paste(const StandardArray& _sa, const DModCoords& _bl) {
    DModCoords tr = _bl + _sa.m_dimensions - std::make_pair(1,1);
    size_t i = 0; // current index of sa

    DModCoords coords = std::make_pair(_bl.first, tr.second);
    // iterate over rows
    // tdout << "pasted: ";
    for (; coords.second != _bl.second-1; --coords.second) {
        coords.first = _bl.first; // reset each loop to start at beginning of the row
...
DModCoords SparseBoard::nextTileByRowOrder(const DModCoords& _start, bool _reverse, bool _colReversed) const {
    DModCoords current = _start;
...
DModCoords SparseBoard::nextTileByColOrder(const DModCoords& _start, bool _reverse, bool _rowReversed) const {
    DModCoords current = _start;
    // tdout << "nextTiles search:" << current;
